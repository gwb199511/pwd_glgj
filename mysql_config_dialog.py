...
from PyQt5.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QFormLayout, 
                           ...
                           QMessageBox, QApplication, QCheckBox, QFrame, QGroupBox)
from PyQt5.QtCore import Qt, pyqtSignal

from config import COLORS, FONT_FAMILY
from ui_components import ModernButton, ModernLineEdit, ModernLabel, HorizontalLine, show_message
...
class MySQLConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 添加说明
        desc_label = ModernLabel("配置MySQL数据库连接参数，所有数据都将存储在MySQL数据库中。", 
    ...
    def update_buttons_state(self):
        ...
        self.cancel_button.setEnabled(has_required_fields)
    ...
    def initialize_database(self):
        ...
        def worker():
            ...
            if success:
                ...
                show_message(self, "初始化成功", "数据库和表结构初始化成功！已创建默认用户admin（密码：admin）", QMessageBox.Information)
            else:
    ...
    def _create_default_users(self):
        ...
        try:
            ...
            from data_storage import get_user_storage, get_password_storage

            # 1. 创建admin用户（如果不存在）
            user_storage = get_user_storage()
            if not user_storage.exists("admin"):
                # 使用UserManager注册管理员用户
                user_manager = UserManager()
                # 注册用户（使用固定密码）
                user_manager.register("admin", "admin")
                logger.info("已创建默认管理员用户：admin")
            
            # 2. 确保三个固定人员的密码记录存在
            personnel = ["徐国明", "高文彬", "石帆"]
            password_storage = get_password_storage()
            
            for person in personnel:
                if not password_storage.exists(person):
                    # 创建示例密码记录
                    sql = """
                    ...
                    """
                    params = (person, "示例项目", "功能说明", "127.0.0.1", "username", "password", "北京", "内网", "备注信息")
                    db_manager.execute_insert(sql, params)
                    logger.info(f"为人员 {person} 创建了示例数据")
